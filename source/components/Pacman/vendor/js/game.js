...
function ready() { 
	LOCK = true;
	message(Ossn.Print('com:pacman:board:ready'));
...
function pauseGame() { 
	...
	if (!PAUSE) { 
		...
		message(Ossn.Print('com:pacman:board:pause'));
...
function gameover() { 
	GAMEOVER = true;
	message(Ossn.Print('com:pacman:board:game:over'));
	stopTimes();
...
function message(m) { 
	$("#message").html(m);
	if (m === Ossn.Print('com:pacman:board:game:over')) $("#message").addClass("red");
}
